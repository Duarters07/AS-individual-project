...
### 2.4 Nop.Web.Framework — Infra-estrutura Web

Camada intermédia entre os serviços e a apresentação. Contém filtros MVC, validação, routing, temas e — criticamente — o registo da stack de observabilidade (`ObservabilityStartup`). É o único projecto com dependências directas do OTel SDK (`OpenTelemetry.*`).

### 2.5 Nop.Web — Camada de Apresentação
...
### 2.6 Plugins — Extensibilidade Dinâmica
...
| Camada | Referencia | Não pode referenciar |
|---|---|---|
| `Nop.Core` | ninguém | todos os outros |
| `Nop.Data` | `Nop.Core` | `Nop.Services`, `Nop.Web.*` |
| `Nop.Services` | `Nop.Core`, `Nop.Data` | `Nop.Web.*` |
| `Nop.Web.Framework` | `Nop.Core`, `Nop.Data`, `Nop.Services` | — |
| `Nop.Web` | todos | — |
| Plugins | `Nop.Web` (transitivo) | — |
...
- **Chamadas directas por injecção de dependências**: O caminho principal. Controllers chamam Services, Services chamam Repositories. É directo e rastreável.

- **IEventPublisher — eventos de domínio in-process**: Qualquer componente pode publicar um evento (`await _eventPublisher.PublishAsync(new OrderPlacedEvent(order))`), e todos os `IConsumer<T>` registados são invocados automaticamente. É o mecanismo que desacopla, por exemplo, a invalidação de cache da lógica de negócio. Ver [IEventPublisher.md](./IEventPublisher.md) para detalhe.
